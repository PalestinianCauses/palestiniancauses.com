...
export const BlogsSystems: CollectionConfig = {
  slug: "blogs-systems",
  access: {
    admin: ({ req }) =>
      hasPermission(req.user, { resource: "blogs-systems", action: "manage" }),
    create: hasPermissionAccess({
      resource: "blogs-systems",
      action: "create",
    }),
    read: ({ req }) =>
      hasPermissionAccess({ resource: "blogs-systems", action: "read" })({
        req,
      }) || isSelf("systemOwner")({ req }),
    update: ({ req }) =>
      hasPermissionAccess({ resource: "blogs-systems", action: "update" })({
        req,
      }) || isSelf("systemOwner")({ req }),
    delete: ({ req }) =>
      hasPermissionAccess({ resource: "blogs-systems", action: "delete" })({
        req,
      }) || isSelf("systemOwner")({ req }),
  },
  admin: {
